...
describe("ProjectRegistry", function () {
  ...
  describe("test: updateProgramMetadata", async function () {
    ...
    it("SHOULD revert WHEN caller is not owner", async function () {
      const project = await this.contract.projects(0);
      await expect(this.contract.connect(this.nonOwner).updateProgramMetadata(project.id, updatedMetadata)).to.be.revertedWith("PR000");
    });
  ...
  describe("test: addProjectOwner", async function () {
  ...
  describe("test: createRound", () => {
    beforeEach(async function () {
      ...
      const RoundFactory = await hre.ethers.getContractFactory("RoundFactory", this.owner);
    ...
    it("SHOULD revert WHEN caller is not project owner", async function () {
      ...
      const tx = this.contract.connect(this.nonOwner).createRound(
    ...
    it("SHOULD revert WHEN program metadata is not set", async function () {
      ...
      const projectID = 0;
      const data = randomBytes(32);
      const tx = this.contract.connect(this.owner).createRound(
        this.roundFactoryContract.address,
        projectID,
        data
      );
