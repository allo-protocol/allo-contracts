...
import { SimpleStrategy } from "../../typechain";
...
const programTestMetadata: MetaPtr = {
  protocol: 1,
  pointer: "test-program-metadata",
};
const updatedMetadata: MetaPtr = { protocol: 1, pointer: "updated-metadata" };

describe("Registry", function () {
  before(async function () {
    [this.owner, this.anotherOwner, this.nonOwner, ...this.accounts] =
  ...
  describe("test: createProject", async function () {
    it("creates a new project and adds it to the projects list with 1 owner", async function () {
      ...
      const event = receipt.events.find(
        (event: any) => event.event === "ProjectCreated"
      );
      ...
      it("ensure events are emitted", () => {
        expect(tx)
          .to.emit(this.contract, "ProjectCreated")
          .withArgs(projectID, this.owner.address);
        expect(tx)
          .to.emit(this.contract, "MetadataUpdated")
          .withArgs(projectID, project.projectMetadata, 0);
        expect(tx)
          .to.emit(this.contract, "MetadataUpdated")
          .withArgs(projectID, project.programMetadata, 1);
      });
    });
  ...
  describe("test: updateProjectMetadata", async function () {
    ...
    it("updates project metadata", async function () {
      ...
      expect(tx)
        .to.emit(this.contract, "MetadataUpdated")
        .withArgs(projectID, updatedProject.projectMetadata, 0);
    });
  });
  ...
  describe("test: updateProgramMetadata", async function () {
    ...
    it("updates project metadata", async function () {
      ...
      expect(tx)
        .to.emit(this.contract, "MetadataUpdated")
        .withArgs(projectID, updatedProject.programMetadata, 1);
    });
  });
  ...
  describe("test: addOwner", () => {
    ...
    it("adds owner to project", async function () {
      expect(await this.contract.getRoleMemberCount(projectID)).to.equal(1);
      expect(
        await this.contract.hasRole(projectID, this.anotherOwner.address)
      ).to.equal(false);
      ...
      expect(await this.contract.getRoleMember(projectID, 1)).to.equal(
        this.anotherOwner.address
      );
      expect(
        await this.contract.hasRole(projectID, this.anotherOwner.address)
  ...
  describe("test: createRound", () => {
    ...
    it("SHOULD revert WHEN caller is not project owner", async function () {
      const strategyFactory = await ethers.getContractFactory("SimpleStrategy");
      ...
      const tx = this.contract
        .connect(this.nonOwner)
        .createRound(
          this.roundFactoryContract.address,
          projectID,
