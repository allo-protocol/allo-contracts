...
contract RoundImplementation is AccessControlEnumerable, Initializable {
    ...
    bytes32[] public projectApplications;
    mapping(bytes32 => uint256) public applicationIds;
    mapping(uint256 => uint256) public applicationStates;
    ...
    event PayFeeAndEscrowFundsToPayoutContract(
        uint256 matchAmountAfterFees,
    ...
    function updateRoundFeePercentage(
    ...
    function updateRoundFeeAddress(
        address payable newFeeAddress
    ) external roundHasNotEnded onlyRole(ROUND_OPERATOR_ROLE) {
    ...
    /// @param _newApplicationMetaPtr appliction metaPtr
    function applyToRound(
        bytes32 _projectId,
        MetaPtr calldata _newApplicationMetaPtr
    ) external {
        // slither-disable-next-line timestamp
        require(
            applicationsStartTime <= block.timestamp &&
        ...
        uint256 applicationId = applicationIds[_projectId];

        // add to application array and set new applicationId
        // skipped on re-application/update
        if (applicationId == 0) {
            projectApplications.push(_projectId);
            applicationId = projectApplications.length;
            applicationIds[_projectId] = applicationId;
        }

        _setApplicationState(applicationId, ApplicationState.PENDING);

        // todo: tbd: change of event
        emit NewProjectApplication(_projectId, _newApplicationMetaPtr);
    }
    ...
    function _setApplicationState(
        ...
        (uint256 stateIndex, uint256 position) = _getMatrixPosition(
            _applicationId
        );

        uint256 currentState = applicationStates[stateIndex];

        // set old state to 00
        currentState = currentState & ~(3 << position);

        // set new state
        applicationStates[stateIndex] =
            currentState |
            (uint256(_state) << position);
    }

    function _getMatrixPosition(
        uint256 _applicationId
    ) internal pure returns (uint256, uint256) {
        uint256 index = _applicationId - 1;
        uint256 stateIndex = (index) / 128; // row of the applicationState where the project is in
        uint256 position = (index % 128) * 2; // position of the project in the row

        return (stateIndex, position);
    }

    function getApplicationState(
        bytes32 _projectApplication
    ) external view returns (uint256) {
        (uint256 stateIndex, uint256 position) = _getMatrixPosition(
            applicationIds[_projectApplication]
        );
    ...
    function _setProjectState(uint256 _index, uint256 _newState) internal {
        applicationStates[_index] = _newState;
        // todo: add event here
    }
        ...
        onlyRole(ROUND_OPERATOR_ROLE)
    {
        ...
        uint256 protocolFeeAmount = ((matchAmount *
            roundFactory.protocolFeePercentage()) / 100);
        ...
        emit PayFeeAndEscrowFundsToPayoutContract(
            fundsInContract,
            protocolFeeAmount,
            roundFeeAmount
        );
